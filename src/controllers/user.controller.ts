...
export const getUsersNotFriends = async (req: Request, res: Response) => {
    try {
        const userEmail = req.query.user;
        const user = await UserModel.findOne({ email: userEmail }, { _id: 0, __v: 0 });
        if (!user) return res.status(400).json("User not found");
        const users = await UserModel.find(
            { friendlist: { $nin: user.email } },
...
export const postAddFriend = async (req: Request, res: Response) => {
    try {
        ...
        await UserModel.findOneAndUpdate({ email: userEmail }, { $addToSet: { friendlist: friendEmail } });
        await UserModel.findOneAndUpdate({ email: friendEmail }, { $addToSet: { friendlist: userEmail } });
...
export const getUserProfile = async (req: Request, res: Response) => {
    try {
        ...
        if(!username) return res.status(404).json("Invalid request");
        const user = UserModel.findOne({username: username}, {password: 0, __v: 0});
